...
def buy_sell(ticker, candle, price, quantity, signal, stop_loss, take_profit):
    ...
    if not os.path.exists("backtest/transaction/NYSE/{}/transaction.csv".format(today_date)):
        if not os.path.exists("backtest/transaction/NYSE/{}".format(today_date)):
            os.makedirs("backtest/transaction/NYSE/{}".format(today_date))
        test_df=pd.DataFrame(columns=["Ticker","Datetime","Price","Quantity","Action","Stop_Loss","Take_Profit"])
        test_df.to_csv("backtest/transaction/NYSE/{}/transaction.csv".format(today_date))
    
    transaction_df=pd.read_csv("backtest/transaction/NYSE/{}/transaction.csv".format(today_date),index_col=0)
    ...
    if position >0 and signal=='Long':
        ...
        transaction_df.to_csv("backtest/transaction/NYSE/{}/transaction.csv".format(today_date))
...
def check_sl_tp(ticker,watch):
    try:
        today_date=datetime.today().strftime('%Y-%m-%d')
        if os.path.exists("backtest/transaction/NYSE/{}/transaction.csv".format(today_date)):
            transaction_df=pd.read_csv("backtest/transaction/NYSE/{}/transaction.csv".format(today_date),index_col=0)
            transaction_df=transaction_df.reset_index(drop=True)
            position=transaction_df.loc[transaction_df['Ticker']==ticker]["Quantity"].sum()
            if position!=0:
...
def calculate_pl(ticker, watch):
    try:
        today_date=datetime.today().strftime('%Y-%m-%d')
        if os.path.exists("backtest/transaction/NYSE/{}/transaction.csv".format(today_date)):
            if os.path.exists("backtest/transaction/NYSE/{}/pl_position.csv".format(today_date)):
                pl_df=pd.read_csv("backtest/transaction/NYSE/{}/pl_position.csv".format(today_date),index_col=0)
            else:
                pl_df=pd.read_csv("backtest/transaction/NYSE/{}/transaction.csv".format(today_date),index_col=0)
            ...
            pl_df.to_csv("backtest/transaction/NYSE/{}/pl_position.csv".format(today_date))
    except:
...
def main(tickers,capital):
    ...
    for ticker in tickers:
        ...
        try:
            ...
            if not os.path.exists("backtest/price_data/NYSE/{}".format(today_date)):
                os.makedirs("backtest/price_data/NYSE/{}".format(today_date))
            data.to_csv("backtest/price_data/NYSE/{}/{}.csv".format(today_date,ticker))
...
#Stock from S&P 500
tickers=["MSFT","AAPL","AMZN","NVDA","GOOG","META","GOOGL","BRK-B","ACN","UNH","LLY","JPM","MA","WMT","ACN","AMD","INTC","WFC","QCOM","CAT","SBUX","PFE","TSLA","NFLX"]
